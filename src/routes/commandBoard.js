...
const asMember = (doc) => ({
  ...
  coveringFor: (doc.coveringFor || []).map(String),
...
router.put('/members/:email', async (req, res, next) => {
  try {
    ...
    const set = {
      ...
      // Only people on the board, never themselves.
      coveringFor: (
        await HubMember.find({
          email: { $in: list(body.coveringFor).map((e) => e.toLowerCase()).filter((e) => e !== email) },
        })
          .select('email')
          .lean()
      ).map((m) => m.email),
